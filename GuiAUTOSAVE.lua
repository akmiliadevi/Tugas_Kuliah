...
-- Tambahkan SETELAH "Module Loading" dan SEBELUM "Main Page Features"
-- ============================================
...
-- Tambahkan ini SETELAH semua makeToggle/makeInput dibuat
-- ============================================
...
local function ApplyLoadedConfig()
    ...
    -- Instant Fishing
    currentInstantMode = GetConfigValue("InstantFishing.Mode", "None")
    fishingDelayValue = GetConfigValue("InstantFishing.FishingDelay", 1.30)
    cancelDelayValue = GetConfigValue("InstantFishing.CancelDelay", 0.19)
    isInstantFishingEnabled = GetConfigValue("InstantFishing.Enabled", false)
    ...
    -- Support Features (apply saved states)
    ...
    local autoSellEnabled = GetConfigValue("Shop.AutoSellTimer.Enabled", false)
    local autoSellInterval = GetConfigValue("Shop.AutoSellTimer.Interval", 5)
    if autoSellEnabled and AutoSellTimer then
        AutoSellTimer.SetInterval(autoSellInterval)
        AutoSellTimer.Start(autoSellInterval)
    end
    
    -- Webhook
    if WebhookModule then
        local webhookURL = GetConfigValue("Webhook.URL", "")
        local webhookDiscordID = GetConfigValue("Webhook.DiscordID", "")
        local webhookRarities = GetConfigValue("Webhook.EnabledRarities", {})
        local webhookEnabled = GetConfigValue("Webhook.Enabled", false)
        
        if webhookURL ~= "" then
            currentWebhookURL = webhookURL
            webhookTextBox.Text = webhookURL
            pcall(function() WebhookModule:SetWebhookURL(webhookURL) end)
        end
        
        if webhookDiscordID ~= "" then
            currentDiscordID = webhookDiscordID
            discordIDTextBox.Text = webhookDiscordID
            pcall(function() WebhookModule:SetDiscordUserID(webhookDiscordID) end)
        end
        
        if #webhookRarities > 0 then
            pcall(function() WebhookModule:SetEnabledRarities(webhookRarities) end)
            rarityCheckboxSystem.SelectSpecific(webhookRarities)
        end
        
        if webhookEnabled then
            pcall(function() WebhookModule:Start() end)
        end
    end
    
    -- Camera View
    if GetConfigValue("CameraView.UnlimitedZoom", false) then
        local UnlimitedZoomModule = GetModule("UnlimitedZoomModule")
        if UnlimitedZoomModule then UnlimitedZoomModule.Enable() end
    end
    
    local freecamSpeed = GetConfigValue("CameraView.Freecam.Speed", 50)
    local freecamSensitivity = GetConfigValue("CameraView.Freecam.Sensitivity", 0.3)
    if FreecamModule then
        FreecamModule.SetSpeed(freecamSpeed)
        FreecamModule.SetSensitivity(freecamSensitivity)
    end
    
    -- Settings - Anti AFK
    if GetConfigValue("Settings.AntiAFK", false) then
        if AntiAFK then AntiAFK.Start() end
    end
    
    -- Settings - FPS Booster
    if GetConfigValue("Settings.FPSBooster", false) then
        if FPSBooster then FPSBooster.Enable() end
    end
    
    -- Settings - Disable Rendering
    if GetConfigValue("Settings.DisableRendering", false) then
        if DisableRenderingModule then DisableRenderingModule.Start() end
    end
    
    -- Settings - FPS Limit
    local fpsLimit = GetConfigValue("Settings.FPSLimit", 60)
    if UnlockFPS then UnlockFPS.SetCap(fpsLimit) end
    
    -- Settings - Hide Stats
    local hideStatsEnabled = GetConfigValue("Settings.HideStats.Enabled", false)
    local hideStatsFakeName = GetConfigValue("Settings.HideStats.FakeName", "Guest")
    local hideStatsFakeLevel = GetConfigValue("Settings.HideStats.FakeLevel", "1")
    
    if HideStats then
        currentFakeName = hideStatsFakeName
        currentFakeLevel = hideStatsFakeLevel
        fakeNameTextBox.Text = hideStatsFakeName
        fakeLevelTextBox.Text = hideStatsFakeLevel
        
        if hideStatsEnabled then
            pcall(function()
                HideStats.SetFakeName(hideStatsFakeName)
                HideStats.SetFakeLevel(hideStatsFakeLevel)
                HideStats.Enable()
            end)
        end
    end
...
-- ============================================
-- UPDATE EXISTING TOGGLES/INPUTS TO SAVE CONFIG
-- Modifikasi semua makeToggle dan makeInput yang ada
-- ============================================

-- CONTOH: Update Instant Fishing Toggle
-- Ganti toggle yang ada dengan ini:
...
-- CONTOH: Update Fishing Delay Input
-- Ganti input yang ada dengan ini:
makeInput(catAutoFishing, "Fishing Delay", GetConfigValue("InstantFishing.FishingDelay", 1.30), function(v)
    fishingDelayValue = v
...
-- ============================================
-- ADD CONFIG BUTTONS TO SETTINGS PAGE
-- Tambahkan ini di Settings Page
-- ============================================
...
-- ============================================
-- APPLY CONFIG ON GUI LOAD
-- Tambahkan ini di AKHIR script, sebelum success message
-- ============================================
...
-- ============================================
-- MAIN PAGE FEATURES (WITH AUTO-SAVE)
-- ============================================
...
local isInstantFishingEnabled = GetConfigValue("InstantFishing.Enabled", false)
...
-- ============================================
-- TELEPORT PAGE (WITH AUTO-SAVE)
-- ============================================

-- Event Teleport dengan Auto-Save
...
-- ============================================
-- WEBHOOK PAGE (WITH AUTO-SAVE)
-- ============================================

-- Webhook URL Input dengan Auto-Save
...
-- Discord ID Input dengan Auto-Save
...
-- Rarity Checkbox dengan Auto-Save
...
-- ============================================
-- CAMERA VIEW PAGE (WITH AUTO-SAVE)
-- ============================================
...
makeButton(catConfig, "💾 Save Current Config", function()
    if ConfigSystem then
        ...
        if success then
            ...
            SendNotification("Config Error", message, 3)
        end
    else
...
makeButton(catConfig, "🔃 Reset to Default", function()
    if ConfigSystem then
        local success, message = ConfigSystem.Reset()
        if success then
            SendNotification("Config", "✓ Reset to defaults! Reload GUI.", 3)
        else
            SendNotification("Error", message, 3)
        end
    else
        SendNotification("Error", "ConfigSystem not loaded!", 3)
    end
...
-- ============================================
-- APPLY LOADED CONFIG ON GUI START
-- ============================================
...
local function ApplyLoadedConfig()
    ...
    -- Apply saved toggle states if they were enabled
    if GetConfigValue("InstantFishing.Enabled", false) and isInstantFishingEnabled then
        local instant = GetModule("instant")
        local instant2 = GetModule("instant2")
        if currentInstantMode == "Fast" and instant then
            instant.Start()
    ...
    if WebhookModule then
        ...
        if savedURL ~= "" then
            ...
            webhookTextBox.Text = savedURL
        end
        
        if savedID ~= "" then
            currentDiscordID = savedID
            discordIDTextBox.Text = savedID
        end
    ...
    if HideStats then
        ...
        currentFakeName = savedName
        currentFakeLevel = savedLevel
        fakeNameTextBox.Text = savedName
        fakeLevelTextBox.Text = savedLevel
    end
