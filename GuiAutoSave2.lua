...
local function makeToggle(parent, label, callback)
    ...
    return {toggle = btn, setOn = function(val) if on ~= val then btn.MouseButton1Click:Fire() end end}
end
...
local instantToggle = makeToggle(catAutoFishing, "Enable Instant Fishing", function(on)
...
local function ApplyLoadedConfig()
    ...
    if HideStats then
        ...
        if GetConfigValue("Settings.HideStats.Enabled", false) then
            task.spawn(function()
                task.wait(0.5)
                pcall(function()
