...
local function checkExecutorCompatibility()
    local compatibility = {
        ...
        setClipboard = pcall(function() return setclipboard end) and setclipboard ~= nil,
        isFile = pcall(function() return isfile end) and isfile ~= nil,
        writeFile = pcall(function() return writefile end) and writefile ~= nil,
        readFile = pcall(function() return readfile end) and readfile ~= nil,
        makeFolder = pcall(function() return makefolder end) and makefolder ~= nil,
    }
    
    return compatibility
end

local executorCompat = checkExecutorCompatibility()
...
ConnectionManager:Add(linkButton.MouseButton1Click:Connect(function()
    if executorCompat.setClipboard then
        local success = pcall(function()
        ...
        if success then
            ...
            linkButton.Text = "⚠️ Clipboard error"
            task.wait(2)
            linkButton.Text = "🔗 Discord: https://discord.gg/6Rpvm2gQ"
        end
    else
        linkButton.Text = "⚠️ Clipboard not supported"
        task.wait(2)
        linkButton.Text = "🔗 Discord: https://discord.gg/6Rpvm2gQ"
    end
end))
...
if not executorCompat.writeFile or not executorCompat.readFile then
    task.wait(1)
    SendNotification("⚠️ Warning", "Config save/load may not work on this executor", 5)
end
...
-- Executor compatibility status
print("━━━━━━━━━━━━━━━━━━━━━━")
print("🔧 Executor Compatibility:")
print("   setclipboard: " .. (executorCompat.setClipboard and "✅" or "❌"))
print("   file system: " .. (executorCompat.isFile and executorCompat.writeFile and "✅" or "❌"))
